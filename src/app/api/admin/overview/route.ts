...
export async function GET() {
  ...
  const { admin } = context;
  const [profiles, authUsers, invites, sources, runs, feedback] = await Promise.all([
    admin.from("profiles").select("id,display_name,is_admin,ai_daily_limit,created_at").order("created_at"),
    admin.auth.admin.listUsers({ page: 1, perPage: 200 }),
    admin.from("invites").select("id,email,expires_at,used_at,created_at").order("created_at", { ascending: false }).limit(50),
  ...
  const failure = [profiles.error, authUsers.error, invites.error, sources.error, runs.error, feedback.error].find(Boolean);
