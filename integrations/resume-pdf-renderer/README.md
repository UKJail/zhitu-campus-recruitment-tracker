...
The inspector uses Poppler's `-bbox-layout` line coordinates. Both physical-row and default reading-order extraction interleaved the real QA fixture's independent columns. Fixed whitespace cuts separate vertical gutters (at least 12 pt), or horizontal regions (at least 8 pt when no vertical cut exists), without consulting the expected DOCX text. Every word must belong to a line and be retained exactly once; ambiguous regions retain row order and may still fail the exact-text gate. Bounding output, coordinates, line/word counts and recursion are bounded. This requires the full Python standard library, verified during image build; Debian's `python3-minimal` lacks XML modules.

The downstream exact-run and character-count checks are unchanged. There is no fallback that selects a different extraction merely to obtain a passing result. Run `python3 -B integrations/resume-pdf-renderer/test_inspect_pdf.py` for the inspector contract, and repeat the real five-fixture conversion and visual review after changing this code; mocked extraction tests alone are not acceptance evidence. Spatial grouping remains a text-integrity check, not an ATS or visual-fidelity certificate. Poppler option behavior is documented in the [Debian package manual](https://manpages.debian.org/trixie/poppler-utils/pdftotext.1.en.html).
