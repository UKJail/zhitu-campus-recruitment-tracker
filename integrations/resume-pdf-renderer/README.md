...
The inspector uses Poppler's default reading-order extraction, not `-layout` (physical rows can interleave independent columns) or `-raw` content-stream order. The downstream exact-run and character-count checks are unchanged. There is no fallback that selects a different extraction merely to obtain a passing result. Run `python3 -B integrations/resume-pdf-renderer/test_inspect_pdf.py` for the inspector contract, and repeat the real five-fixture conversion and visual review after changing this code; mocked extraction tests alone are not acceptance evidence.
