...
# 2026-08-17 求职偏好弹窗布局修复

- 根因是岗位方向和城市的复杂选项使用 `fieldset` 后，在 CSS Grid 中出现高度计算异常，选项按钮溢出并覆盖后续表单。
- 将复杂选项改为带无障碍分组标签的普通分区容器，确保每个区域按真实内容撑开。
- 弹窗改为固定可用高度、标题区固定、表单内部独立滚动、底部操作栏吸附；同步提高标签与输入文字字号。
- 手机端使用接近全屏的安全边距，选项网格继续按屏幕宽度降为三列或两列。

# 2026-08-17 求职偏好填写方式简化

- 按用户复验反馈移除岗位方向和城市的标签按钮网格，改为普通文本输入，避免弹窗信息拥挤和布局错乱。
- 求职偏好仅保留毕业届别、招聘类型、岗位方向、意向城市和关注公司；删除“排除关键词”选项，保存时同步清空历史排除词。
- 岗位方向和城市支持用逗号、顿号或换行分隔；后台继续自动完成关键词匹配，用户无需理解职位库标签。
- 同义词扩展由单向改为双向，例如填写“商业分析”也能识别“数据分析、经营分析、BI”等同一方向岗位。
- 最终验证：36 个测试文件、138 项测试通过；ESLint 与 Next.js 16.3.0 生产构建通过。
- 本阶段未合并 `main`、未覆盖正式站、未执行 Supabase 迁移或修改 Resend Webhook。
